...
def sync_cluster(config):
  ...
  for section in ("general", config.get('performance', 'profile')):
    for (name, value) in config.items(section):
      if name not in ('proxy_hostname', 'proxy_socks_port'):
        vars_d[name] = value
  cloud_provider = vars_d.get('cloud_provider', 'aws-ec2')
  if cloud_provider  == 'aws-ec2':
  ...
  if cloud_provider == 'baremetal':
    ...
    devs = vars_d['mount_devices'].split(",")
    for node_type in 'default', 'worker':
        node_type_map[node_type] = {'mounts': mounts, 'devices': mounts}
    vars_d["node_type_map"] = str(node_type_map)
    vars_d["metrics_drive_ids"] = str(vars_d["metric_drive_ids"].split(","))
